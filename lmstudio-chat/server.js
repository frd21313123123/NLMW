...
const OPENROUTER_FREE_MODELS = [
  {
    ...
    context: 32768,
    description: "Uncensored Mistral 24B fine-tune by Venice.ai"
  },
  {
    id: "nousresearch/deephermes-3-llama-3-8b-preview:free",
    name: "DeepHermes 3 Llama 3 8B",
    context: 131072,
    description: "Uncensored reasoning model by NousResearch"
  },
  {
    id: "mistralai/mistral-small-3.1-24b-instruct:free",
    name: "Mistral Small 3.1 24B",
    context: 131072,
    description: "Mistral Small 3.1 — fast multimodal model"
  },
  {
    id: "meta-llama/llama-4-maverick:free",
    name: "Llama 4 Maverick",
    context: 131072,
    description: "Meta Llama 4 Maverick (free on OpenRouter)"
  },
  {
    id: "meta-llama/llama-4-scout:free",
    name: "Llama 4 Scout",
    context: 524288,
    description: "Meta Llama 4 Scout (free on OpenRouter)"
  },
  {
    id: "deepseek/deepseek-chat-v3-0324:free",
    name: "DeepSeek V3 Chat",
    context: 131072,
    description: "DeepSeek V3 — strong general model"
  },
  {
...
app.get("/api/openrouter/models", async (req, res) => {
  ...
  try {
    const headers = openrouterHeaders(clientKey);
    ...
    if (!upstream.ok) {
      // Return hardcoded free models as fallback.
      res.json({
        data: OPENROUTER_FREE_MODELS.map((m) => ({
          id: m.id,
          name: m.name,
    ...
    // Filter to free models only (id ends with :free or pricing is 0).
    const freeModels = allModels.filter((m) => {
      if (typeof m.id === "string" && m.id.endsWith(":free")) return true;
      if (m.pricing && Number(m.pricing.prompt) === 0 && Number(m.pricing.completion) === 0) return true;
      return false;
    });

    // If no free models found from API, return hardcoded list.
    ...
    res.json({ data: freeModels });
  } catch (err) {
    console.error("[openrouter models]", err);
    // Return hardcoded free models on error.
    res.json({
      data: OPENROUTER_FREE_MODELS.map((m) => ({
        id: m.id,
        name: m.name,
...
app.post("/api/openrouter/chat", async (req, res) => {
  try {
    ...
    const payload = {
      model: typeof body.model === "string" && body.model.trim() ? body.model.trim() : "venice/uncensored:free",
      messages: Array.isArray(body.messages) ? body.messages : [],
